...
/* Easy AES256 testing URL: http://extranet.cryptomathic.com/aescalc/index */
...
#define Sbox(i) (P_Sbox[i])
const unsigned char P_Sbox[256] = {		// forward s-box
...
#define InvSbox(i) (P_InvSbox[i])
const unsigned char P_InvSbox[256] = {	// inverse s-box
...
#define Xtime2Sbox(i) (P_Xtime2Sbox[i])
const unsigned char P_Xtime2Sbox[256] = {
...
#define Xtime3Sbox(i) (P_Xtime3Sbox[i])
const unsigned char P_Xtime3Sbox[256] = {
...
#define Xtime2(i) (P_Xtime2[i])
const unsigned char P_Xtime2[256] = {
...
#define Xtime9(i) (P_Xtime9[i])
const unsigned char P_Xtime9[256] = {
...
#define XtimeB(i) (P_XtimeB[i])
const unsigned char P_XtimeB[256] = {
...
#define XtimeD(i) (P_XtimeD[i])
const unsigned char P_XtimeD[256] = {
...
#define XtimeE(i) (P_XtimeE[i])
const unsigned char P_XtimeE[256] = {
...
static void ExpandKey (const unsigned char *key, unsigned char *expkey)
{
...
void Encrypt (const unsigned char *in, const unsigned char *expkey, unsigned char *out)
{
...
void Decrypt (const unsigned char *in, const unsigned char *expkey, unsigned char *out)
{
    unsigned char state[Nb * 4];
    unsigned round;
...
unsigned char aes_expkey[4 * Nb * (Nr + 1)];
unsigned char aes_iv[16];

void aes_set_key(const unsigned char *key)
{
    ExpandKey(key, aes_expkey);
}
...
void aes_encrypt_cbc_1(const unsigned char *input, unsigned char *output)
{
    ...
    for (n = 0; n < sizeof(aes_iv); n++)
        wrk[n] = input[n] ^ aes_iv[n];

    aes_encrypt_1(wrk, output);
...
void aes_decrypt_cbc_1(const unsigned char *input, unsigned char *output)
{
    ...
    for (n = 0; n < sizeof(wrk); n++)
        wrk[n] = input[n];

    aes_decrypt_1(input, output);
...
void aes_ctr(const unsigned char *input, unsigned char *output)
{
    ...
    /* encrypt the IV, that acts as a counter */
    aes_encrypt_1(aes_iv, ctr);

    /* xor the encrypted ctr with the input */
    for (n = 0; n < sizeof(ctr); n++)
        output[n] = input[n] ^ ctr[n];

    /* increment the IV */
    for (n = 0; ++aes_iv[n] == 0 && n < sizeof(aes_iv); n++);
}
